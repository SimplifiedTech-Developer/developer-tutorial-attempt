...
import za.co.telkom.keycloak.config.SecurityContextUtils;
...
public class StudentController {
    ...
    public @ResponseBody
    ResponseEntity<Object> getInsecure() {

        try {
            log.info("Started Getting Student Using Insecure Endpoint");
            log.info("Completed Getting Student Using Insecure Endpoint");
            return new ResponseEntity<>("Success Insecure", HttpStatus.OK);
        } catch (Exception e) {
            log.error(CommonEnum.ERROR_MESSAGE.getValue(), e.getMessage());
            CommonResponse commonResponse = logException(e);
            return new ResponseEntity<>(commonResponse, HttpStatus.INTERNAL_SERVER_ERROR);
