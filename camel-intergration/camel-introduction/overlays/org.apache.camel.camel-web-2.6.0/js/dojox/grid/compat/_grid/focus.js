...
if (!dojo._hasResource["dojox.grid.compat._grid.focus"]) {
    ...
    dojo.declare("dojox.grid.focus", null, {
        constructor: function (_1) {
            ...
            var c = this.cell, s = c.view.scrollboxNode,
                sr = {w: s.clientWidth, l: s.scrollLeft, t: s.scrollTop, h: s.clientHeight},
                n = c.getNode(this.rowIndex), r = c.view.getRowNode(this.rowIndex),
            ...
            var rc = this.grid.rowCount - 1, cc = this.grid.layout.cellCount - 1, r = this.rowIndex,
                i = this.cell.index, row = Math.min(rc, Math.max(0, r + _17)), col = Math.min(cc, Math.max(0, i + _18));
            this.setFocusIndex(row, col);
            if (_17) {
